...
/*==================================================================================================================
This script was modified by Jack Ridley 06.06.2022 for use with the LeapMotion-OSC Interface Unity Application.
All of my changes I have marked with comments that begin with my initials written thus: "(jr)" so that they can be 
easily searched and understood.
=================================================================================================================*/
...
//{
  ...
  public class OSC : MonoBehaviour
  {
    ...
    // second in port (jr)
    public int inPort2  = 6262;
    public string outIP = "127.0.0.1";
    public int outPort  = 6161;
    // second out port (jr)
    public int outPort2  = 6448;

      private UDPPacketIO OscPacketIO;
    // second IO packet (jr)  
    private UDPPacketIO OscPacketIO2;
      Thread ReadThread;
    ...
    // (jr) split into a separate function from the Awake() function, in order to enable it to be accessed from Reset()
    // without interferring with Input/Read system
    void Init()
    {
        OscPacketIO = new UDPPacketIO(outIP, outPort, inPort);
        // init second OscPacket (jr)
        OscPacketIO2 = new UDPPacketIO(outIP, outPort2, inPort2);
    }

    // (jr) sets new values to outPort1 and outPort2 so that user can specify Out Ports
    public void SetPorts(int outport1, int outport2)
    {
        outPort = outport1;
        outPort2 = outport2;
    ...
    // (jr) Resets OSC IO Packets so that they update to new out ports
    ...
    void Awake() {
        ...
        // (jr) encapsulated IO initialising to a separate function
        Init();
    ...
    // (jr) moved these actions to a separate function from their original place in Close() so that they
    // could be called without interferring with the Input/Read system
	public void CloseOutputs()
    {
        ...
        // (jr) duplicated functionality for second OscPacketIO
        if (OscPacketIO2 != null && OscPacketIO2.IsOpen())
        {
            OscPacketIO2.Close();
            OscPacketIO2 = null;
            print("Closed OSC listener 2");
        }
    }
    ...
    */
    public void Close()
    {
        ...
        // (jr) encapsulated functionality into a separate function call
        CloseOutputs();
    }
    ...
    public void Send( OscMessage oscMessage, int senderNum=1 )
    {
        // edited to have senderNum param, where any int other than the default (1) causes message to be sent via
        // the second defined port. (jr)
        if(senderNum == 1)
        {
        ...
        else
        {
            byte[] packet = new byte[1000];
            int length = OSC.OscMessageToPacket(oscMessage, packet, 1000);
            OscPacketIO2.SendPacket(packet, length);
        }
